...
def create_user_story(db:Session, user_story_data: CreateUserStory) -> UserStory:
...
def update_user_story(db:Session, story_id:uuid.UUID, user_story_data: UpdateUserStory) -> UserStory:
    user_story = db.query(UserStory).filter(UserStory.id == story_id).first()
    if not user_story:
